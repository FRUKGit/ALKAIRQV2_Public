...
boolean isNormalMode() {
  return mode == DISPLAY_MODE_NORMAL;
}
boolean isGraphMode() {
  return mode == DISPLAY_MODE_GRAPH;
}
...
void showHeader() {
  ...
  String textHeader1 = "Cast AirQ 1";
  String textHeader2 = timeString;
  int x = 0;
  canvas.drawString(textHeader1, x, 0);
  canvas.drawString(textHeader2, x + 3, 0);
...
void showNextGraph() {
  if (whichGraph == GRAPH_CO2) {
    ...
    drawGraph(20, 20, display.width() - 40, display.width() - 40, 300, 1600, "    PPM", ppmSaved2p5, false, false, false);
